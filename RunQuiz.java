...
public class RunQuiz {
    ...
    public static void main(String[] args) {
        ...
        // define game title parameters
        double TITLE_X = 0.5;
        double TITLE_Y = 0.65;
        double TITLE_WIDTH = 1.0;
        double TITLE_HEIGHT = 0.3;

        // external files used in project
        String BACKGROUND_MUSIC = "afrobeat.wav";
        String HOME_SCREEN_GIF = "DragonWalking.gif";
        String GAME_TITLE = "GameTitle.png";
        String YELLOW_LANTERNS_BACKGROUND = "YellowLanternsBackground.jpeg";
        String CHIMES = "chimes.wav";
        String CROW = "crow.wav";
        String SHOP_BACKGROUND = "ShopBackground.jpg";
        String FULL_FORTUNE_COOKIE = "FortuneCookieFull.png";
        String SPLIT_FORTUNE_COOKIE = "SplitFortuneCookie.png";
        String STOMP = "stomp.wav";

        // draw dragon home screen gif to StdDraw and play music to StdAudio
        // until start button clicked
        // i. Tests for Homepage (part of feature 1)
        // ii. Input: We launch the program.
        // iii. Expected Outcome: A dragon gif with overlayed music plays on the screen.
        ...
        int j = 1;
        while (true) {
            StdAudio.playInBackground(BACKGROUND_MUSIC);
            while (timer2.elapsedTime() < 5.5 * j) {
                StdDraw.pause(10);
                StdDraw.clear();
                StdDraw.picture(DRAGON_X, DRAGON_Y, HOME_SCREEN_GIF, DRAGON_WIDTH,
                                DRAGON_HEIGHT);
                StdDraw.picture(TITLE_X, TITLE_Y, GAME_TITLE, TITLE_WIDTH, TITLE_HEIGHT);
                startButton.buttonText();
                startButton.drawButton();
                StdDraw.pause(10);
                StdDraw.show();
        ...
        // i. Tests for Start Button (part of feature 1)
        // ii. Input: We click the start button
        // iii. Expected Outcome: We see the next question displayed on the screen,
        // with all answer choices and background.
        // iv. Result: passed (manually tested)
        while (true) {
            ...
            if (startButton.withinBoundaries() || nextButton.withinBoundaries()
                ...
                StdDraw.picture(BACKGROUND_X, BACKGROUND_Y, YELLOW_LANTERNS_BACKGROUND);
                ...
                int TIME_LENGTH = 40000;
                double[] chimes = StdAudio.read(CHIMES);
                double[] copyChimes = new double[TIME_LENGTH];
                for (int i = 0; i < copyChimes.length; i++) {
                    copyChimes[i] = chimes[i];
                }
                ...
                // i. Tests for Question Page (part of feature 1)
                // ii. Input: We click the start button.
                // iii. Expected Outcome: The timer will start at the time limit
                // and tick to 0 if nothing is pressed.
                // iv. Result: passed (manually tested)
                while (!(questionTimer.timeRemainingDouble() == -1.0)) {
                    ...
                    // i. Tests for Answer Buttons (part of feature 1)
                    // ii. Input: We click the answer buttons.
                    // iii. Expected Outcome: We are told if our answer is correct
                    // and a sound effect plays. We gain fortune cookie points.
                    // iv. Result: passed (manually tested)
                ...
                // i. Tests for Questions Page (part of feature 1)
                // ii. Input: We click the shop button or the next button.
                // iii. Expected Outcome: We see the shop screen in our application
                // or we see the next questions page.
                // iv. Result: passed (manually tested)
                while (true) {
                    ...
                    if (shopButton.withinBoundaries() || nextButton.withinBoundaries()) {
                        if (shopButton.withinBoundaries()) {
                            ...
                            StdDraw.picture(SHOP_BACK_X, SHOP_BACK_Y, SHOP_BACKGROUND,
                            ...
                            StdDraw.picture(FORTUNE_COOKIE_PICTURE_X,
                                            ...
                                            FULL_FORTUNE_COOKIE, FORTUNE_COOKIE_WIDTH,
                                            FORTUNE_COOKIE_HEIGHT);

                            StdDraw.setPenColor(Color.BLACK);

                            // i. Tests for Shop Page (part of feature 1)
                            // ii. Input: We click the buy button
                            // iii. Expected Outcome: A fortune cookie animation plays
                            // and we see the fortune cookie Markov output text.
                            // iv. Result: passed (manually tested)
                            while (true) {
                                ...
                                if (buyButton.withinBoundaries()) {
                                    ...
                                    StdDraw.picture(SPLIT_FORTUNE_X,
                                                    SPLIT_FORTUNE_Y,
                                                    SPLIT_FORTUNE_COOKIE,
                                                    SPLIT_FORTUNE_WIDTH,
                                                    SPLIT_FORTUNE_HEIGHT);

                                    StdAudio.playInBackground(STOMP);
                                    ...
                                    String DESIRED_FONT = "Sans Serif";
                                    Font font = new Font(DESIRED_FONT, Font.ITALIC, 16);
                                ...
                                // i. Tests for Shop Page (part of feature 1)
                                // ii. Input: We click the back button.
                                // iii. Expected Outcome: We return to the questions page
                                // iv. Result: passed (manually tested)
